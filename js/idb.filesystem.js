...
function MyFile(opts) {
  console.log("in MyFile constructor");
  var blob_ = null;
...
function FileWriter(fileEntry) {
    console.log("in FileWriter constructor");
  if (!fileEntry) {
  ...
  this.write = function(data) {
    ...
    reader.onloadend = function(loadedEvent){
        fileEntry.file_.blob_.uri = loadedEvent.target.result;
        console.log("converted blob to uri");
    ...
    reader.readAsDataURL(fileEntry.file_.blob_);
  };
}
...
function FileEntry(opt_fileEntry) {
  console.log("in FileEntry ctor with:");
  console.dir(opt_fileEntry);
  this.file_ = null;
  ...
  if (opt_fileEntry) {
      console.log("have opt_fileEntry");
    ...
    if (this.file_.blob_ && this.file_.blob_.uri) {
        try {
        var dataURL = this.file_.blob_.uri;
        ...
        this.file_.blob_ = new Blob([uInt8Array.buffer], {type: contentType});
       } catch (error) {
            console.log("error converting string:");
            console.dir(error);
        }
    } else {
        console.log("do not have blob uri; have:");
        console.dir(this.file_);
    }
  } else {
      console.log("danger will robinson, NO opt_fileEntry");
  }
}
...
function DirectoryEntry(opt_folderEntry) {
    console.log("in DirectoryEntry ctor");
  Object.defineProperty(this, 'isFile', {
    enumerable: true,
    get: function() {
      return false;
...
DirectoryEntry.prototype.getFile = function(path, options, successCallback,
                                            opt_errorCallback) {
  console.log("in DirectoryEntry.getFile");
  // Create an absolute path if we were handed a relative one.
  path = resolveToFullPath_(this.fullPath, path);
...
idb_.getAllEntries = function(fullPath, successCallback, opt_errorCallback) {
  console.log("in idb_.getAllEntries");
  if (!this.db) {
    return;
  }
  ...
  request.onsuccess = function(e) {
    ...
    if (cursor) {
      ...
      console.dir(results[results.length-1]);
      cursor['continue']();
    }
  };
};
